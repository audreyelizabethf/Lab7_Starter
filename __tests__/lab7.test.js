describe('Basic user flow for Website', () => {
  ...
  it('Make sure <product-item> elements are populated', async () => {
   ...
   {
    console.log(`Checking product ${i+1}/${prodItemsData.length}`);
    const product = prodItemsData[i];
    if(product.title.length==0 || product.price.length==0 || product.image.length ==0)
    {
  ...
  it('Checking number of items in cart on screen', async () => {
    ...
    const cartCountValue = await cartCountElement.getProperty('innerText');
    const cartCount = await cartCountValue.jsonValue();
